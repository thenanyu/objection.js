...
function (_QueryBuilderOperatio) {
  _inherits(FindOperation, _QueryBuilderOperatio);

  function FindOperation() {
    _classCallCheck(this, FindOperation);

    return _possibleConstructorReturn(this, _getPrototypeOf(FindOperation).apply(this, arguments));
  }

  _createClass(FindOperation, [{
    key: "onAfter3",
    value: function onAfter3(builder, results) {
      var opt = builder.findOptions();
...
function callAfterGetForRelations(ctx, model, results) {
  ...
  for (var i = 0, l = keys.length; i < l; ++i) {
    ...
    if (isRelation(value)) {
      var maybePromise = callAfterGet(ctx, value, true);
...
function isRelation(value) {
  return isObject(value) && value.$isObjectionModel || isNonEmptyObjectArray(value) && value[0].$isObjectionModel;
}
