...
var ObjectionToKnexConvertingOperation = require('./ObjectionToKnexConvertingOperation'); // This class's only purpose is to normalize the arguments into an array.
//
...
var ReturningOperation =
/*#__PURE__*/
function (_ObjectionToKnexConve) {
  _inherits(ReturningOperation, _ObjectionToKnexConve);

  function ReturningOperation() {
    _classCallCheck(this, ReturningOperation);

    return _possibleConstructorReturn(this, _getPrototypeOf(ReturningOperation).apply(this, arguments));
  }

  _createClass(ReturningOperation, [{
    key: "onAdd",
    value: function onAdd(builder, args) {
      args = flatten(args); // Don't add an empty returning list.

      if (args.length === 0) {
        return false;
      }

      return _get(_getPrototypeOf(ReturningOperation.prototype), "onAdd", this).call(this, builder, args);
    }
  }, {
    key: "onBuildKnex",
