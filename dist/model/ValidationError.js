...
function (_Error) {
  ...
  function ValidationError(_ref) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(ValidationError).call(this, message || errorsToMessage(data)));
...
function errorsToMessage(data) {
  return Object.keys(data).reduce(function (messages, key) {
    messages.push("".concat(key, ": ").concat(asArray(data[key]).map(message).join(', ')));
    return messages;
  }, []).join(', ');
}
